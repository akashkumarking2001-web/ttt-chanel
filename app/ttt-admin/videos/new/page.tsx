...
type VideoType = "youtube" | "cloudflare" | "blogger";
...
export default function NewVideo() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!videoUrl) { toast.error("Please add a video URL or upload a video"); return; }
    setSaving(true);
    try {
          ...
          <div>
            <div className="admin-card">
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                {([["youtube", "YouTube", <PlayCircle key="y" size={16} />], ["cloudflare", "Cloudflare", <CloudUpload key="c" size={16} />], ["blogger", "Direct URL", <Link2 key="b" size={16} />]] as const).map(([t, l, icon]) => (
